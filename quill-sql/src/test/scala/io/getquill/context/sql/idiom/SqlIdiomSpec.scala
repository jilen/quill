  ...
  "shows the sql representation of normalized asts" - {
    "query" - {
      ...
      "union" - {
        ...
        "with sort" in {
          val q = quote {
            qr1.filter(t => t.i > 10).sortBy(_.i)(Ord.asc).union(qr1.filter(t => t.s == "s").sortBy(_.i)(Ord.desc))
          }
          testContext.run(q).sql mustEqual
            "SELECT x.s, x.i, x.l, x.o FROM ((SELECT t.s, t.i, t.l, t.o FROM TestEntity t WHERE t.i > 10 ORDER BY t.i ASC) UNION (SELECT t1.s, t1.i, t1.l, t1.o FROM TestEntity t1 WHERE t1.s = 's' ORDER BY t.i ASC)) x"
        }
      }
      "unionAll" - {
        "simple" in {
